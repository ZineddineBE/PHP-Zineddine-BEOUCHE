...
$query = $db->prepare(
    'SELECT * FROM booking INNER JOIN booking_room ON booking_room.id_booking = booking.id_booking INNER JOIN room ON booking_room.id_room = room.id_room WHERE booking.id_booking = :id'
);
...
$optionsQuery = $db->prepare(
    'SELECT * FROM booking_option INNER JOIN opt_in ON booking_option.id_option = opt_in.id_option WHERE booking_option.id_booking = :id'
);
...
$userId = 1;
$previousQuery = $db->prepare('SELECT * FROM booking WHERE id_user = :user_id');
